...
import { bookingUrl, brand, contact, images, navigation, socialLinks } from '../lib/siteData';
  ...
  return (
    <footer className="relative z-10 bg-sage px-5 py-14 text-ivory sm:px-8 lg:px-14">
      <div className="mx-auto grid w-full max-w-[104rem] gap-12 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          <p className="font-ui text-xs font-semibold uppercase tracking-[0.18em] text-ivory/60">
            Soggiorni, benessere, cucina e territorio
          </p>
          <Link to="/" className="mt-6 inline-flex" aria-label={brand.name}>
            <img src={images.logo.light} alt={brand.name} className="h-20 w-auto object-contain sm:h-24" />
          </Link>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link
              to="/contatti"
              className="border border-white/22 px-5 py-3 font-ui text-xs font-semibold uppercase tracking-[0.14em] text-white transition-colors hover:bg-white/10"
            >
              Chiedi informazioni
            </Link>
          </div>
        ...
        <div className="grid gap-8 sm:grid-cols-2">
          <div>
            <p className="font-ui text-xs font-semibold uppercase tracking-[0.16em] text-ivory/60">Info</p>
          ...
          <div>
            <p className="font-ui text-xs font-semibold uppercase tracking-[0.16em] text-ivory/60">Link utili</p>
            <div className="mt-4 grid gap-2">
              {navigation.slice(1).map((item) => (
                <NavLink
                  key={item.path}
                  to={item.path}
                  className="font-body text-sm text-ivory/78 transition-colors hover:text-white"
                >
                  {item.label}
                </NavLink>
              ))}
      ...
      <div className="mx-auto mt-12 flex w-full max-w-[104rem] flex-col gap-3 border-t border-white/12 pt-5 font-ui text-xs uppercase tracking-[0.12em] text-ivory/54 sm:flex-row sm:items-center sm:justify-between">
